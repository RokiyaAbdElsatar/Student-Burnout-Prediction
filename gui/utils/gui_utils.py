...
# Project root constant - derived from this file's location (gui/utils/)
...
def load_comparison_data():
    ...
    csv_path = os.path.join(PROJECT_ROOT, 'results', 'comparison.csv')
    return pd.read_csv(csv_path)

def load_results_json(result_type='built_in'):
    """Load results from JSON file."""
    if result_type == 'built_in':
        path = os.path.join(PROJECT_ROOT, 'results', 'built_in_results.json')
    else:
        path = os.path.join(PROJECT_ROOT, 'results', 'from_scratch_results.json')
...
def load_dataset_sample(nrows=10):
    ...
    path = os.path.join(PROJECT_ROOT, 'dataset', 'pain_dataset_200P_4hz.csv')
    return pd.read_csv(path, nrows=nrows)

def load_aggregated_data():
    """Load and aggregate dataset."""
    sys_path = os.path.join(PROJECT_ROOT, 'utils')
